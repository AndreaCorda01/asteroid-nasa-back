...
class MockAsteroidProxy extends AsteroidProxy implements IAsteroidProxy {
  getAsteroidListFromAPI(_: Date, __: Date): Promise<Asteroid[]> {
  ...
  getAsteroidDetailsFromAPI(_: number): Promise<Asteroid> {
...
describe('AppController', () => {
  ...
  beforeEach(async () => {
    const app: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(AsteroidProxy)
      .useClass(MockAsteroidProxy)
      .compile();
  ...
  describe('Asteroid Listing', () => {
    it('Should return the list of all asteroid in the json file', async () => {
      const asteroids = await asteroidsController.list({
